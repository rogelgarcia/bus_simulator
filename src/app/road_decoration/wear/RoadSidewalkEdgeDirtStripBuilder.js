// src/app/road_decoration/wear/RoadSidewalkEdgeDirtStripBuilder.js
// Builds an outward strip mesh along sidewalk outer edges for grass transition dirt overlays.
...
export function buildRoadSidewalkEdgeDirtStripMeshDataFromRoadEnginePrimitives(primitives, {
    surfaceY = 0,
    curbThickness = 0.48,
    sidewalkWidth = 1.875,
    stripWidth = 0.65,
    ...
    const width = Math.max(0, clampNumber(stripWidth, 0.65));
    ...
    const sidewalkOffset = Math.max(0, clampNumber(curbThickness, 0.48)) + Math.max(0, clampNumber(sidewalkWidth, 1.875));
    const stripOuterOffset = sidewalkOffset + width;
    ...
    for (const loop of loops) {
        const roadBoundary = normalizePointList(loop, { epsilon: boundaryEpsilon, forceCcw: false });
        if (roadBoundary.length < 3) continue;

        const sidewalkOuter = offsetLoop(roadBoundary, sidewalkOffset, { miterLimit, epsilon: boundaryEpsilon });
        const stripOuter = offsetLoop(roadBoundary, stripOuterOffset, { miterLimit, epsilon: boundaryEpsilon });
        if (sidewalkOuter.length !== roadBoundary.length || stripOuter.length !== roadBoundary.length) continue;

        const uByIndex = new Array(sidewalkOuter.length).fill(0);
        let acc = 0;
        for (let i = 1; i < sidewalkOuter.length; i++) {
            acc += dist(sidewalkOuter[i - 1], sidewalkOuter[i]);
            uByIndex[i] = acc;
        }
        const perimeter = acc + dist(sidewalkOuter[sidewalkOuter.length - 1], sidewalkOuter[0]);

        for (let i = 0; i < sidewalkOuter.length; i++) {
            const j = (i + 1) % sidewalkOuter.length;
            const ia = sidewalkOuter[i];
            const ib = sidewalkOuter[j];
            const oa = stripOuter[i];
            const ob = stripOuter[j];
            if (distSq(ia, ib) <= EPS || distSq(oa, ob) <= EPS) continue;

            const a = { x: ia.x, y, z: ia.z };
            const b = { x: ib.x, y, z: ib.z };
            const c = { x: ob.x, y, z: ob.z };
            const d = { x: oa.x, y, z: oa.z };

            const u0 = uByIndex[i];
            const u1 = j === 0 ? perimeter : uByIndex[j];

            // Match sidewalk strip winding so faces render from above.
            pushTriWithUvs(
                positions,
                uvs,
                a,
