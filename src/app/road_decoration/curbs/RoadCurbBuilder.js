// src/app/road_decoration/curbs/RoadCurbBuilder.js
// Generates curb mesh data from RoadEngine polygon primitives by extracting the asphalt boundary and offsetting it outward.
...
export function buildRoadCurbMeshDataFromRoadEnginePrimitives(primitives, {
    surfaceY = 0,
    curbThickness = 0.48,
    curbHeight = 0.17,
    curbExtraHeight = 0,
    ...
    const thickness = Math.max(0, clampNumber(curbThickness, 0.48));
    const baseY = clampNumber(surfaceY, 0);
    const topY = baseY + Math.max(0, clampNumber(curbHeight, 0.17)) + Math.max(0, clampNumber(curbExtraHeight, 0));
    const bottomY = baseY - Math.max(0, clampNumber(curbSink, 0));

    const positions = [];
    for (const loop of loops) {
        const pts = normalizePointList(loop, { epsilon: boundaryEpsilon, forceCcw: false });
        if (pts.length < 3) continue;
        const outer = offsetLoop(pts, thickness, { miterLimit, epsilon: boundaryEpsilon });
        if (outer.length !== pts.length) continue;
        ...
        for (let i = 0; i < pts.length; i++) {
            ...
            const aoTop = { x: ao.x, y: topY, z: ao.z };
            const boTop = { x: bo.x, y: topY, z: bo.z };
            const aBot = { x: a.x, y: bottomY, z: a.z };
            const bBot = { x: b.x, y: bottomY, z: b.z };
