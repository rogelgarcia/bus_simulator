...
export function buildRoadSidewalkMeshDataFromRoadEnginePrimitives(primitives, {
    ...
    const topY = baseY + curbH + Math.max(0, clampNumber(sidewalkLift, 0));
    const innerOffset = startFromCurb === false ? 0 : Math.max(0, clampNumber(curbThickness, 0.48));
    const width = Math.max(0, clampNumber(sidewalkWidth, 1.875));
    const outerOffset = innerOffset + width;

    const positions = [];
    for (const loop of loops) {
        const pts = normalizePointList(loop, { epsilon: boundaryEpsilon, forceCcw: false });
        if (pts.length < 3) continue;
        const inner = offsetLoop(pts, innerOffset, { miterLimit, epsilon: boundaryEpsilon });
        const outer = offsetLoop(pts, outerOffset, { miterLimit, epsilon: boundaryEpsilon });
        if (inner.length !== pts.length || outer.length !== pts.length) continue;
        ...
        for (let i = 0; i < pts.length; i++) {
            ...
            const aBot = { x: a.x, y: baseY, z: a.z };
            const bBot = { x: b.x, y: baseY, z: b.z };
