// src/graphics/gui/wall_decoration_mesh_debugger/main.js
// Standalone Wall Decoration Mesh Debugger tool entry point.
...
import { WallDecorationMeshDebuggerView } from './view/WallDecorationMeshDebuggerView.js';

const canvas = document.getElementById('game-canvas');
if (!canvas) throw new Error('[WallDecorationMeshDebugger] Missing canvas#game-canvas');
...
const view = new WallDecorationMeshDebuggerView({ canvas });
view.start().then(() => {
    if (view.renderer) perfBar.setRenderer(view.renderer);
    view.onFrame = ({ dt, nowMs }) => perfBar.onFrame({ dt, nowMs });
}).catch((err) => {
    console.error('[WallDecorationMeshDebugger] Failed to start', err);
});
