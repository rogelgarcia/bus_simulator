...
export function renderGraphicsTab() {
    ...
    const shadowInstancedCasters = makeChoiceRow({
        label: 'Instanced detail shadows',
        value: shadows?.instancedCasters === true ? 'on' : 'off',
        options: [
            { id: 'off', label: 'Off' },
            { id: 'on', label: 'On' }
        ],
        onChange: (v) => {
            shadows.instancedCasters = v === 'on';
            emit();
        }
    });
    ...
    sectionShadows.appendChild(shadowInstancedCasters.row);
