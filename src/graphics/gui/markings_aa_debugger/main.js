// src/graphics/gui/markings_aa_debugger/main.js
// Standalone Markings AA Debug tool entry point.
...
const view = new MarkingsAADebuggerView({ canvas });
view.start().then(() => {
    if (view.renderer) perfBar.setRenderer(view.renderer);
    view.onFrame = (frame) => perfBar.onFrame(frame);
}).catch((err) => {
    console.error('[MarkingsAADebugger] Failed to start', err);
});
