// src/graphics/assets3d/procedural_meshes/skeletons/MeshSkeletonSchema.js
// Defines a declarative schema format for mesh skeleton controls.

export const SKELETON_PROPERTY_TYPE = Object.freeze({
    ENUM: 'enum',
    NUMBER: 'number'
});
...
export function createEnumProperty({ id, label, options, defaultValue } = {}) {
    ...
    if (!safeId) throw new Error('[MeshSkeletonSchema] Enum property id must be a non-empty string.');
    ...
    const safeOptions = list
        ...
        .filter((opt) => opt.id);
    if (safeOptions.length === 0) throw new Error(`[MeshSkeletonSchema] Enum property "${safeId}" must define at least one option.`);
    ...
    return Object.freeze({
        type: SKELETON_PROPERTY_TYPE.ENUM,
        id: safeId,
        label: safeLabel,
...
export function createNumberProperty({ id, label, min = 0, max = 1, step = 0.01, defaultValue = 0 } = {}) {
    const safeId = typeof id === 'string' ? id : '';
    if (!safeId) throw new Error('[MeshSkeletonSchema] Number property id must be a non-empty string.');
    const safeLabel = typeof label === 'string' && label ? label : safeId;
    ...
    return Object.freeze({
        type: SKELETON_PROPERTY_TYPE.NUMBER,
        id: safeId,
        label: safeLabel,
        min: safeMin,
        max: safeMax,
...
export function isMeshSkeletonApi(api) {
    return !!api
        && typeof api === 'object'
        && !!api.schema
