...
import { planReceiverCoplanarOwnership } from '../../../app/illumination/receiver_lightmaps/ReceiverCoplanarOwnership.js';
...
export function repairEnhancedReceiverCoplanarGeometry(geometry, original, mapping) {
    ...
    for(let i=first;i<end;i++)if([0,1,2].every(c=>coordinate.getW(vertex(i*3+c))>.5))eligible[i]=materialIndices[i];
    ...
    if(!plan) {
        ...
        for(let i=0;i<count*3;i++){const v=vertex(i);positions.set([source.getX(v),source.getY(v),source.getZ(v)],i*3);}
        plan=planReceiverCoplanarOwnership(positions,eligible);cache.set(original,plan);
    }
