// Captures the recessed-balcony Terra & Mar variant with one visible HDRI
// used for both the background and the reflection source.
...
const OUT_DIR = path.resolve(__dirname, '../../../artifacts/screens/buildings/terramar_recessed');
...
const COMMON_OPTIONS = Object.freeze({
    seed: 'terramar-recessed-reference',
    buildingId: 'terramar_recessed',
...
async function loadShot(page, shot) {
    console.log(`[terramar_recessed_capture] loading ${shot.name} at ${VIEWPORT.width}x${VIEWPORT.height}`);
...
test('Capture: Terra & Mar recessed balconies UHD HDRI reference poses', async ({ page }) => {
    ...
    for (const shot of SHOTS) {
        ...
        expect(metrics?.scenario?.buildingId).toBe('terramar_recessed');
