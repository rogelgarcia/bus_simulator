// Captures B Glass with one visible HDRI used for both background and reflections.
...
const OUT_DIR = path.resolve(__dirname, '../../../artifacts/screens/buildings/bglass');
const captureScale = Math.max(1, Math.min(3, Number(process.env.BGLASS_CAPTURE_SCALE) || 3));
...
const HDRI = Object.freeze({
    ...
    backgroundRotationDeg: 210,
    environmentRotationDeg: 128
});
const COMMON_OPTIONS = Object.freeze({
    seed: 'bglass-reference',
    buildingId: 'bglass',
...
const SHOTS = Object.freeze([
    ...
    },
    {
        ...
        cameraDir: Object.freeze({ x: -0.82, y: 0.14, z: 1 }),
        cameraPadding: 1.12,
        cameraTargetYFrac: 0.48
    },
    {
        name: 'low-angle-closeup.png',
        cameraDir: Object.freeze({ x: -0.25, y: -0.4, z: 1 }),
        cameraPadding: 0.7,
...
async function loadShot(page, shot) {
    console.log(`[bglass_capture] loading ${shot.name} at ${VIEWPORT.width}x${VIEWPORT.height}`);
...
test('Capture: B Glass UHD HDRI reference poses', async ({ page }) => {
    ...
    for (const shot of SHOTS) {
        ...
        expect(metrics?.scenario?.buildingId).toBe('bglass');
