...
test('AO scope UI follows effective indirect and retains both parameter banks',async({page})=>{
    ...
    const dynamic=page.locator('.options-row').filter({has:page.locator('.options-row-label',{hasText:/^Dynamic AO$/})});
    ...
    const grounding=page.locator('.options-row').filter({has:page.locator('.options-row-label',{hasText:/^Bus grounding$/})});
    await expect(grounding.getByRole('button',{name:'Underbody',exact:true})).toHaveClass(/is-active/);
    await grounding.getByRole('button',{name:'GTAO',exact:true}).click();
    ...
    expect(saved.dynamic.busMethod).toBe('gtao');
