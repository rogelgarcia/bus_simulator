...
test('BF2: corner resolution uses odd-over-even winner depth', async ({ page }) => {
    ...
    const delta = await page.evaluate(({ baselineItems, changedItems }) => {
        ...
        const getWallMaxX = () => {
            ...
            solidGroup.traverse((obj) => {
                ...
                if (Array.isArray(obj.material) && obj.material.length === 2) wallMesh = obj;
            });
