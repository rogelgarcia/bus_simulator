...
test('AI 532 captures current and hybrid shadows for two visible interacting movers', async ({ page }) => {
    test.setTimeout(90_000);
    const pageErrors = [];
    const consoleErrors = [];
    page.on('pageerror', (error) => pageErrors.push(String(error?.message ?? error)));
    ...
    await page.goto('/tests/headless/harness/index.html');
    await page.waitForFunction(() => !!window.__testHooks?.getEngine?.());
    await page.evaluate(() => window.__testHooks.loadScenario('empty', { seed: 'ai532-visual-pair' }));
    ...
    const result = await page.evaluate(async () => {
        ...
        const pointDirection = new THREE.Vector3(0.8, 0.6, -0.34).normalize();
        const scene = new THREE.Scene();
        scene.background = new THREE.Color(0x9eafc4);
        ...
        const redRoofMaterial = new THREE.MeshPhysicalMaterial({
            color: 0xe0aa3e,
            roughness: 0.42,
            clearcoat: 0.18
        });
        ...
        const blueTopMaterial = new THREE.MeshPhysicalMaterial({
            color: 0x62a9c7,
            roughness: 0.38,
            clearcoat: 0.25
        });
        ...
        ground.name = 'SharedWorldReceiver';

        const moverA = new THREE.Group();
        moverA.name = 'MoverA_RedVehicle';
        moverA.position.set(-1.9, 0, 0.9);
        const bodyA = new THREE.Mesh(new THREE.BoxGeometry(3.4, 1.25, 1.55), redMaterial);
        bodyA.position.y = 0.7;
        const roofA = new THREE.Mesh(new THREE.BoxGeometry(2.2, 0.42, 1.35), redRoofMaterial);
        roofA.position.set(0.25, 1.54, 0);
        moverA.add(bodyA, roofA);

        const moverB = new THREE.Group();
        moverB.name = 'MoverB_BlueVehicle';
        moverB.position.set(1.4, 0, -0.6);
        const bodyB = new THREE.Mesh(new THREE.BoxGeometry(1.35, 3.1, 1.35), blueMaterial);
        bodyB.position.y = 1.55;
        const topB = new THREE.Mesh(new THREE.SphereGeometry(0.62, 24, 16), blueTopMaterial);
        topB.position.y = 3.32;
        moverB.add(bodyB, topB);

        for (const root of [moverA, moverB]) {
        ...
        sun.position.copy(pointDirection).multiplyScalar(12);
        sun.target.position.set(0, 0, 0);
        sun.castShadow = true;
        ...
        const hemi = new THREE.HemisphereLight(0xc7dcff, 0x4a4436, 0.75);
        scene.add(ground, moverA, moverB, sun, sun.target, hemi);

        const camera = new THREE.PerspectiveCamera(43, 16 / 9, 0.1, 50);
        camera.position.set(9.5, 7.2, 11.5);
        camera.lookAt(-0.2, 0.9, 0.05);
        camera.updateMatrixWorld(true);
        ...
        const renderCapture = () => {
            engine.renderer.setRenderTarget(target);
            engine.renderer.setClearColor(scene.background, 1);
            engine.renderer.clear(true, true, true);
            engine.renderer.render(scene, camera);
            const pixels = new Uint8Array(width * height * 4);
            engine.renderer.readRenderTargetPixels(target, 0, 0, width, height, pixels);
            ...
            for (let offset = 0; offset < pixels.length; offset += 4) {
                if (pixels[offset] < 120 && pixels[offset + 1] < 120 && pixels[offset + 2] < 120) {
                    darkPixelCount += 1;
                }
            }
            context.putImageData(image, 0, 0);
            return { png: canvas.toDataURL('image/png'), pixels, darkPixelCount };
        };

        sun.shadow.needsUpdate = true;
        const current = renderCapture();
        ...
        const descriptor = {
            ...
            identity: {
                channelId: 'static_sun_depth',
                channelVersion: 1,
                cityId: 'fixture.ai532.visual',
                casterInventorySha256: '1'.repeat(64),
                channelSourceSha256: '2'.repeat(64),
                compilerSignatureSha256: '3'.repeat(64),
                sunPointDirectionWorld: pointDirection.toArray(),
                basis: staticApp.createStableStaticSunDepthBasis(pointDirection.toArray(), [0, 0, 0]),
                layout: {
                    order: 'row-major-y-then-x-v1',
                    lookup: 'half-open-min-inclusive-max-exclusive-v1',
                    rowOrigin: 'min-light-y-v1',
                    ...
                    guardTexels: 1,
                    texelSizeMeters: 5,
                    boundsLightMeters: { min: [-10, -10], max: [10, 10] }
                },
                alpha: {
                    model: 'evaluated-runtime-coverage-v1',
                    coverage: 'opacity-times-vertex-alpha-times-map-a-times-alpha-map-g-v1',
            ...
            tiles: [{
                id: 'tile.0.0',
                coordinates: [0, 0],
                interiorBoundsLightMeters: { min: [-10, -10], max: [10, 10] },
                storedTexels: [6, 6],
                contentSha256: '5'.repeat(64)
            }]
        };
        ...
        const dynamicLayer = new DynamicSunShadowLayer(engine.renderer, {
            mapSize: 512,
            worldUnitsPerTexel: 0.03,
            paddingTexels: 8,
            receiverMinimumY: 0,
            depthPaddingMeters: 1.5,
            constantBiasMeters: 0.012,
            normalBiasMeters: 0.024
        });
        const handleA = dynamicLayer.register({ id: 'mover.red', root: moverA });
        const handleB = dynamicLayer.register({ id: 'mover.blue', root: moverB });
        dynamicLayer.activate();
        const binding = staticGraphics.createStaticSunDepthShaderBinding({
            descriptor,
            texture: staticTexture,
        ...
        const materialSet = new staticGraphics.StaticSunDepthMaterialSet();
        materialSet.prepareRoots([ground, moverA, moverB], binding, { outsideRoot: scene });
        materialSet.activate();
        binding.setDynamicShadowState(dynamicLayer.render(pointDirection.toArray()));
        binding.updateCamera(camera);
        sun.shadow.needsUpdate = true;
        engine.renderer.compile(scene, camera);
        const hybrid = renderCapture();
        const diagnostics = dynamicLayer.getDiagnostics();

        let absoluteDifference = 0;
        for (let offset = 0; offset < current.pixels.length; offset += 4) {
            absoluteDifference += Math.abs(current.pixels[offset] - hybrid.pixels[offset]);
            absoluteDifference += Math.abs(current.pixels[offset + 1] - hybrid.pixels[offset + 1]);
            absoluteDifference += Math.abs(current.pixels[offset + 2] - hybrid.pixels[offset + 2]);
        }
        const meanRgbDifference = absoluteDifference / (width * height * 3);
        ...
        roofA.geometry.dispose();
        bodyB.geometry.dispose();
        topB.geometry.dispose();
        groundMaterial.dispose();
        redMaterial.dispose();
        redRoofMaterial.dispose();
        blueMaterial.dispose();
        blueTopMaterial.dispose();

        return {
            currentCapture: current.png,
            hybridCapture: hybrid.png,
            currentDarkPixels: current.darkPixelCount,
            hybridDarkPixels: hybrid.darkPixelCount,
            meanRgbDifference,
            registrationIds: diagnostics.registrations.map((entry) => entry.id),
            casterIds: diagnostics.map.projection.casterIds,
            casterMeshCount: diagnostics.metrics.casterMeshCount
        };
    });

    await writeCapture('two_movers_before_current.png', result.currentCapture);
    await writeCapture('two_movers_after_hybrid.png', result.hybridCapture);
    expect(result.registrationIds).toEqual(['mover.blue', 'mover.red']);
    expect(result.casterIds).toEqual(['mover.blue', 'mover.red']);
    expect(result.casterMeshCount).toBe(4);
    expect(result.currentDarkPixels).toBeGreaterThan(0);
    expect(result.hybridDarkPixels).toBeGreaterThan(0);
    expect(result.meanRgbDifference).toBeLessThan(20);
    expect(pageErrors).toEqual([]);
    expect(consoleErrors.filter((message) => /shader error|VALIDATE_STATUS/i.test(message))).toEqual([]);
});
