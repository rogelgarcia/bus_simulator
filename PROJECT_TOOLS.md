...
| receiverLightmaps | `tools/receiver_lightmaps/` | Bake scalar/directional Cycles atlases, validate overlay coverage, repack comparisons, calibrate angular response, measure precision and report controlled runs | `node tools/receiver_lightmaps/run.mjs --pages 4 --samples 64` (`--enhanced true` for AI 548); see README for `validate_coverage.py`, `measure_precision.mjs` and `report_548.mjs` |
