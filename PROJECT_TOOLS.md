...
| staticSunDepth | `tools/static_sun_depth/` | Compile guarded RG8 fixtures; build authenticated production caches; resume the deterministic Part A workflow; and run strict production/Lab Scene current-versus-cache validation | `node tools/static_sun_depth/run.mjs --help`; `node tools/static_sun_depth/finish_part_a.mjs --help`; `node tools/static_sun_depth/validate_lab.mjs --help` |
