...
| receiverLightmaps | `tools/receiver_lightmaps/` | Build complete receiver UV layouts, bake Cycles irradiance with CPU/OptiX, audit transport and publish authenticated maps | Start with `unwrap.mjs`, then `run.mjs --enhanced true --layout ... --atlas-only true`; see README for complete coverage, `validate_surface_transport.py`, `validate_environment_orientation.py` and unpublished city checks |
