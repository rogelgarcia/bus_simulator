...
| streamedShadowDetail | `tools/bake_lighting/shadows/streamed/` | Authenticated native 3x detail pages, bounded city-wide generation, shared guards, recorded-route comparisons and gated publication | `node tools/bake.mjs --target lighting/shadows/streamed/city`; `/prototype` limits coverage; `/review` validates images, cold/warm timings and corruption fallback; `/publish --publish` installs reviewed full-city detail; `/filter-review`, `/parent-control`, `/toggle-review` isolate regressions; `/five-pose-review` captures 15 images and repeated GPU/CPU p1/p99, counters and allocation estimates; see README |
| lightingConfigurations | `tools/bake_lighting/experiments/lighting_configurations/` | AI 560 automated game baselines, reusable Blender city, six-light Cycles matrix, tone variants, analysis, 4K shortlist and pose-grouped gallery | `node tools/bake_lighting/experiments/lighting_configurations/run.mjs`; see its README |
| physicalLightingCalibration | `tools/bake_lighting/experiments/physical_calibration/` | AI 564 analytical Cycles/native fixtures, measured Cornell provenance, CIE axis subset and raw/color-pipeline validation | `node tools/bake.mjs --target lighting/experiments/physical-calibration`; see its README for standalone stages and coverage |
