...
- The offline-first grass sequence is tracked by `specs/grass/GRASS_OFFLINE_FIRST_AI_SEQUENCE.md` and prompts AI 350 through AI 362, AI 537, and AI 363.
...
- [x] AI 362 validated every corrected Grass Lab material consumer through the existing shared catalog/calibration pipeline with no new loader, URL resolver, palette, or calibration fork. The approved V2 record proves one shared `MID_CLUSTER` path for billboard/middle, a separate `ACCENT_CLUMP` path, and `pbr.grass_low_cut_maintained_v2`; `specs/grass/GRASS_LAB_APPROVAL_AI362.json` reports scoped `status: "approved"` with `performance.status: "deferred_to_ai537"`, while gameplay remains blocked pending AI 537.
- [ ] AI 537 must preserve the shared catalog/calibration material identities during optimization, record any material or loader-path change here, and mark this item complete only after verification and `specs/grass/GRASS_LAB_PERFORMANCE_APPROVAL_AI537.json` reports `status: "approved"`.
- [ ] AI 363 must migrate the gameplay grass consumer through the same shared catalog/calibration pipeline and remove or deprecate conflicting gameplay-local paths only after both AI 362's scoped visual/functional approval and AI 537's performance approval pass.
...
- The gameplay authorization gates are `specs/grass/GRASS_LAB_APPROVAL_AI362.json` with scoped `status: "approved"` and `performance.status: "deferred_to_ai537"`, plus `specs/grass/GRASS_LAB_PERFORMANCE_APPROVAL_AI537.json` with `status: "approved"`; the AI 357 approval record remains historical.
