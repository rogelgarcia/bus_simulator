# DONE — calibrated game integration executed and reviewed

The accepted 55° clear-afternoon source remains unchanged. The corrected v3
shadow/indirect packages are installed and all five actual-game UHD views have
been reviewed against preserved targets and fresh calibrated Cycles renders.
The full result, identities, commands, diagnostics and remaining limits are in
`specs/graphics/calibrated_game_lighting.md`.

Completed changes:

- Integrated the calibrated sun, disc-free sky, global ACESFilmic exposure and grading Off, with legacy saved overrides preserved.
- Fixed diffuse sky being suppressed by reflection intensity, paused lighting transforms, asphalt tangent normals and the bus display-transform bypass.
- Fixed missing authored UV transforms and Blender clearing generated raw texture pixels; regenerated compatible v3 diffuse transport.
- Added deterministic Blender-compatible gray/beige interior proxies and documented Phong/Principled material equivalence limits.
- Added distance-dependent finite-sun filtering and corrected high-sun native foliage capture/readiness without relaxing parity gates.
- Prepared shader variants asynchronously and reused coherent Auto/Baked resources; repeated bus reflection toggles preserve original materials without world-lighting dropouts.
- Added registered, resumable capture/export/render/diagnostic/install/review stages; retained all original and rejected evidence in ignored artifacts.
- Generated and reviewed `review-04`: 92 images across five poses, original/reference/final comparisons, full-resolution crops and keyboard carousel.

Evidence root: `tests/artifacts/screens/ai562_acesfilmic_reference_matching/runs/match-01/`.
Final game: `iterations/014_installed_4k`; final Cycles:
`references/calibrated-export-06-uv-4k`; installation: `install-02`.
The completed v3 bake is framework run `run-1789044043493-11812-7420a6a0`.

Measured before/after at 1920×1080, RTX3060/ANGLE D3D11, identical locked poses,
60 warm-up and180 sampled frames: original replay005 versus calibrated013.
Lighting changes from original35° to calibrated55°; these are whole workloads,
not isolated shader speedups. Frame time is the mean; GPU values are medians.

| Pose | Frame ms before → after | FPS before → after | GPU ms before → after | Draw calls before → after |
| --- | --- | --- | --- | --- |
| 01 | 24.72 → 19.39 | 40.4 → 51.6 | 11.42 → 10.88 | 896 → 829 |
| 02 | 25.65 → 21.58 | 39.0 → 46.3 | 10.79 → 10.50 | 1080 → 1015 |
| 03 | 22.32 → 18.11 | 44.8 → 55.2 | 9.65 → 10.32 | 856 → 779 |
| 04 | 22.32 → 18.75 | 44.8 → 53.3 | 8.72 → 9.02 | 759 → 772 |
| 05 | 17.87 → 17.60 | 56.0 → 56.8 | 7.01 → 8.36 | 438 → 443 |

The spec includes CPU/GPU p95, triangles, allocations, caveats and separate UHD
costs. All 11 bake stages,14 native fixtures, five installed captures and final
report navigation pass. The last raw-texture correction improves image error
in all five matched 1080p views; the main facade ambient rises42.5% with its
named sun unchanged. Raw material masks still expose residual ambient/BRDF
differences; these are not hidden by an exposure change.

Stage timings: accepted bake45m14s; final Cycles reference17m58s; installed UHD
capture4m01s; report2m41s. Earlier investigations and rejected runs are retained
and excluded from those stage timings. Reflection checks have zero world
dropouts and bounded allocations. The previous23–24s synchronous mode stall is
removed, although first full-mode preparation still holds the last image for
about19s and can have a0.95s frame gap. This is not a stutter-free guarantee.

Conditional direct-RGB restoration was not triggered: live direct BRDF with
finite-source depth reconstruction is retained; no useful separate direct atlas
was demonstrated or restored. A full-city RGB-direct prototype was not
benchmarked. Material AO, legacy Phong environment response, fake interiors,
glass reflections and finite static/moving shadow overlap remain explicit
representation limits. Historical targets are probable L00/+0.5EV matches,
not exact identities, and are not scored against the new55° sunlight.
...
- [x] Add fixed measurements and full-resolution crops for the shaded red-brick facade and ground-floor storefronts on the left, the shaded building on the right, and bus paint/trim in this view. Recover readable material detail and plausible shaded radiance toward the selected reference while preserving real recess/contact darkness, sunlight highlights and the original bus appearance. Measure linear illumination and displayed color separately; do not treat every near-black pixel as an error.
- [x] Keep the selected +0.5-EV Cycles target visible throughout iteration. Also compare game and Cycles at matched exposure, with grading Off, to separate exposure changes from sun/sky/bounce/material corrections. A global exposure adjustment may be retained if it improves the full acceptance set; no pose-specific exposure, lifted-black overlay or local brightness patch. Validate retained changes against both selected targets and all five poses so brighter shaded walls do not wash out the original bright facade.
- [x] Preserve chronological game captures for this target in every retained candidate iteration, with its own baseline/target/final columns and diagnostic crops. Show the actual EV and source-light identity next to every image. White window placeholders and glossy grass in the exported target remain documented material/export limitations; matching their artifacts is not an acceptance goal.
